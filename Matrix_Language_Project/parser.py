...
def p_action_evolution_keyword_instruction(p):
    ...
    p[0] = AST.KeyWordInstruction(keyword=p[1], continuation=p[2] if len(p) > 3 else None)
...
def p_assignement_evolution(p):
    ...
    p[0] = AST.Assignment(lvalue=p[1], operator=p[2], operation=p[3])
